        ...
        <div className={`grid grid-cols-1 gap-6 ${tiers.length === 3 ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}>
          {tiers.map((tier, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className={`glass-card relative p-8 transition-all duration-500 ${
                tier.popular
            ...
            >
              ...
              <Link
                to="/contact"
                className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-6 py-3.5 font-semibold transition-all duration-300 ${
                  tier.popular
                    ? "btn-primary-glow"
                    : "btn-ghost-glow"
                }`}
